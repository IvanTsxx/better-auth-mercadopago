...
|----------|--------|-------------|---------------|
| `/mercado-pago/create-payment` | POST | Creates payment preferences | Yes |
| `/mercado-pago/webhook` | POST | Receives webhook notifications | No |
...
| Threat | Mitigation |
|--------|------------|
| Prototype pollution | Remove `__proto__`, `constructor` keys |
| Excessive size | Truncate strings to 5000 chars |
| Nested injection | Recursively sanitize objects |
...
| Feature | Description |
|---------|-------------|
| **Plugin composition** | Split into sub-plugins (payments, subscriptions, marketplace) |
| **Event system** | Replace callbacks with event emitter pattern |
| **Plugin options validation** | Zod schema for plugin configuration |
| **Database adapters** | Support for more than just Prisma via Better Auth |
...
|----------|-------------|----------|
| `verifyWebhookSignature` | Verifies webhook HMAC signature | `src/security.ts:9` |
| `validatePaymentAmount` | Validates payment amounts match | `src/security.ts:87` |
| `sanitizeMetadata` | Sanitizes user metadata | `src/security.ts:99` |
| `validateIdempotencyKey` | Validates idempotency key format | `src/security.ts:160` |
| `secureCompare` | Timing-safe string comparison | `src/security.ts:172` |

### Types

| Interface | Description | Location |
|-----------|-------------|----------|
| `MercadoPagoPluginOptions` | Plugin configuration options | `src/types.ts:17` |
| `MercadoPagoPaymentRecord` | Database payment record | `src/types.ts:89` |
| `MercadoPagoPaymentNotification` | Webhook notification payload | `src/types.ts:4` |
| `CreatePaymentParams` | Payment creation parameters | `src/types.ts:173` |
| `CreatePaymentResponse` | Payment creation response | `src/types.ts:225` |
